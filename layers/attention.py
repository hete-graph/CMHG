...
class Attention(nn.Module):
    ...
    def forward(self, feat_pos, feat_neg=None, summary=None):
        if feat_neg is None:
    ...
    def attn_feature(self, features):
        ...
        features_attn = F.softmax(torch.cat(features_attn, 1), -1)  # [3015, 2]
        if torch.is_tensor(features):
            features = features.view(-1, self.args.hid_units)
        else:
            features = torch.cat(features, 1).squeeze(0)  # [6050, 64]
    ...
    def attn_summary(self, features):
        ...
        if torch.is_tensor(features):
            ...
            features = torch.cat(features, 0)  # [2,64]
